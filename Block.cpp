	...
	if(floorType > 0 || wallType > 0 || ramp.type > 0 || stairType > 0){
		...
		if(sprite.sheetIndex != INVALID_INDEX)
		{
			...
			if(this->depthBorderNorth)
				line(target, drawx + (TILEWIDTH>>1)-(HORIZONTALSHIFT/2), drawy, drawx+TILEWIDTH-1, drawy+(TILEHEIGHT>>1)+(VERTICALSHIFT/2), tileBorderColor);

			////Western border
			if(this->depthBorderWest)
				line(target, drawx, drawy+(TILEHEIGHT>>1)-(VERTICALSHIFT/2), drawx+(TILEWIDTH>>1)-(HORIZONTALSHIFT/2), drawy, tileBorderColor);

			//Earstern Border
			if(this->depthBorderEast)
				line(target, drawx+TILEWIDTH-1, drawy+(TILEHEIGHT>>1)-(VERTICALSHIFT/2), drawx+(TILEWIDTH>>1)+(HORIZONTALSHIFT/2)-1, drawy, tileBorderColor);
