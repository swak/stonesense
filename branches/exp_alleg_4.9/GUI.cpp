...
//}
//
//void drawDebugCursorAndInfo(ALLEGRO_BITMAP* target){
...
//	int i = 10;
//	al_draw_textf(font, 2, config.screenHeight-20-(i--*10), 0, "Block 0x%x", b);
//	if(!b) return;
//
...
//}
//
//void DrawSpriteFromSheet( int spriteNum, ALLEGRO_BITMAP* target, ALLEGRO_BITMAP* spriteSheet, int x, int y){
//	int sheetx = spriteNum % SHEET_OBJECTSWIDE;
//	int sheety = spriteNum / SHEET_OBJECTSWIDE;
//	//
//	/*
...
//		al_draw_line(i,0,i, config.screenHeight, al_map_rgb(0,0,0), 0);
...
//		}
//	}	
//	al_draw_textf(font, config.screenWidth-10, config.screenHeight -10, ALLEGRO_ALIGN_RIGHT, 
...
//	if(!buffer)
//		buffer = al_create_bitmap(config.screenWidth, config.screenHeight);
//
...
//		al_draw_textf(font, config.screenWidth/2, config.screenHeight/2, ALLEGRO_ALIGN_CENTRE, "Could not find DF process");
//		return;
//	}
//
//
...
//	if (config.show_osd)
//	{
//		al_draw_textf(font, 10,10, 0,0, "%i,%i,%i, r%i", DisplayedSegmentX,DisplayedSegmentY,DisplayedSegmentZ, DisplayedRotation);
//
//		if(config.debug_mode){
//			al_draw_textf(font, 10,20, 0,0, "Timer1: %ims", ClockedTime);
//			al_draw_textf(font, 10,30, 0,0, "Timer2: %ims", ClockedTime2);
//			al_draw_textf(font, 10,40, 0,0, "Draw: %ims", DrawTime);
//			al_draw_textf(font, 10,50, 0,0, "D1: %i", blockFactory.getPoolSize());
//
//			drawDebugCursorAndInfo(buffer);
//		}
//
//		if(config.follow_DFscreen)
//			al_draw_textf(font, config.screenWidth/2,10, ALLEGRO_ALIGN_CENTRE,0, "Locked on DF screen + (%d,%d,%d)",config.viewXoffset,config.viewYoffset,config.viewZoffset);
//		if(config.single_layer_view)
//			al_draw_textf(font, config.screenWidth/2,20, ALLEGRO_ALIGN_CENTRE,0, "Single layer view");
//		if(config.automatic_reload_time)
//			al_draw_textf(font, config.screenWidth/2,30, ALLEGRO_ALIGN_CENTRE,0, "Reloading every %0.1fs", (float)config.automatic_reload_time/1000);
//
//		DrawMinimap(buffer);
//	}
...
//	ALLEGRO_BITMAP* temp = create_bitmap_ex(16, buffer->w, buffer->h);
