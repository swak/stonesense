...
bool addSingleTerrainConfig( TiXmlElement* elemRoot){
  ...
  if(elementType.compare( "borders" ) == 0){
    //parse walls
    TiXmlElement* elemBorder = elemRoot->FirstChildElement("border");
    while( elemBorder ){
      parseWallFloorSpriteElement( elemBorder, contentLoader.terrainBorderConfigs, basefile );
      elemBorder = elemBorder->NextSiblingElement("border");
    }
  }
  return true;
}
