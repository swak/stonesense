...
inline bool isBlockSameType(uint32_t x, uint32_t y, uint32_t z,int floorType, WorldSegment* segment, dirRelative dir)
{
	Block* block = segment->getBlockRelativeTo( x, y, z, dir);
	if(!block) return false;
	if(block->floorType == floorType) return true;
	return false;
}

inline bool isBlockSameMaterial(uint32_t x, uint32_t y, uint32_t z,t_matglossPair material,  WorldSegment* segment, dirRelative dir)
{
	Block* block = segment->getBlockRelativeTo( x, y, z, dir);
	if(!block) return false;
	if(block->material.index == material.index && block->material.type == material.type) return true;
	return false;
}
