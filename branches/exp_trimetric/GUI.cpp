...
{ //assume z of 0
	...
	x+=tileshift;
	static int offx = (config.screenWidth - (((config.segmentSize.x-config.segmentSize.y) * TILEWIDTH / 2) + ((config.segmentSize.x+config.segmentSize.y) * HORIZONTALSHIFT / 2))) / 2;
...
void pointToScreen(int *inx, int *iny, int inz){
	...
	int x = *inx-*iny;
	int y = *inx+*iny;
	int xoff = y * HORIZONTALSHIFT / 2;
	int yoff = x * VERTICALSHIFT /2;
	x = x * TILEWIDTH / 2 + xoff;
	y = y * TILEHEIGHT / 2 + yoff;
	x+=offx;
	y+=offy;
	y-=z * BLOCKHEIGHT;
	*inx=x;*iny=y;
...
void drawDebugCursorAndInfo(BITMAP* target){
	...
	masked_blit(IMGObjectSheet, target,
		...
		point.x - tileshift, point.y - (WALLHEIGHT), SPRITEWIDTH, SPRITEHEIGHT);
