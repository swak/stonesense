...
void parseConfigLine( string line ){
	...
	if( line.find("FOGTYPE") != -1)
	{
		string result = parseStrFromLine( "FOGTYPE", line );
		if( result == "NONE" )
			config.fogtype = fNone;
		else if( result == "LIN" )
			config.fogtype = fLin;
		else if( result == "SIN" )
			config.fogtype = fSin;
			...
			config.fogtype = fLog;
		else if( result == "LOGINV" )
			config.fogtype = fLoginv;
		else config.fogtype = fNone;
	}
}
