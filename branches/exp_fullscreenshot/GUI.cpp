...
{ //assume z of 0
	...
	static int offy = (-20)-(BLOCKHEIGHT * config.lift_segment_offscreen);
	y-=offy;
	x-=offx;
	y1=y*2-x;
	x1=x*2+y1;
...
void pointToScreen(int *inx, int *iny, int inz, int targetWidth){
	...
	{
		offx = targetWidth / 2;
		offy = (-20)-(BLOCKHEIGHT * config.lift_segment_offscreen);
...
Crd2D WorldBlockToScreen(int32_t x, int32_t y, int32_t z, int targetWidth){
	...
	return LocalBlockToScreen(x, y, z-1, targetWidth);
}

Crd2D LocalBlockToScreen(int32_t x, int32_t y, int32_t z, int targetWidth){
	pointToScreen((int*)&x, (int*)&y, z, targetWidth);
	Crd2D result;
	result.x = x;
	result.y = y;
	return result;
...
void DrawCurrentLevelOutline(BITMAP* target, bool backPart){
	...
	Crd2D p1 = WorldBlockToScreen(x, y, z, target->w);
	Crd2D p2 = WorldBlockToScreen(x, y + sizey , z, target->w);
	Crd2D p3 = WorldBlockToScreen(x + sizex , y, z, target->w);
	Crd2D p4 = WorldBlockToScreen(x + sizex , y + sizey , z, target->w);
	p1.y += FLOORHEIGHT;
	p2.y += FLOORHEIGHT;
	p3.y += FLOORHEIGHT;
	p4.y += FLOORHEIGHT;
...
void drawDebugCursorAndInfo(BITMAP* target){
	Crd2D point = LocalBlockToScreen(debugCursor.x, debugCursor.y, 0, target->w);
...
void saveMegashot(){
	...
	BITMAP* bigFile = create_bitmap_ex(24, bigImageWidth, bigImageHeight);
	BITMAP* bigBuffer = create_bitmap(bigImageWidth, bigImageHeight);
	clear_to_color(bigBuffer,makecol(config.backr,config.backg,config.backb));
	viewedSegment->drawAllBlocks(bigBuffer);
	blit(bigBuffer, bigFile, 0, 0, 0, 0, bigImageWidth, bigImageHeight);
	save_png(filename, bigFile, 0);
	destroy_bitmap(bigBuffer);
	destroy_bitmap(bigFile);
