...
void parseConfigLine( string line ){
	...
	if( line.find("FOGMETHOD") != -1){
		string result = parseStrFromLine( "FOGMETHOD", line );
		if(result == "LOG")
			config.blendmethod = bLog;
		else if (result == "LOG_INV")
			config.blendmethod = bLog_inv;
		else
			config.blendmethod = bLinear;
	}
}
